...
const MovieInformation = () => {
  ...
  const addToFavorites = async () => {
    ...
    setIsMovieFavorited((previous) => !previous);
  };
  ...
  const addToWatchlist = async () => {
    ...
    setIsMovieWatchListed((previous) => !previous);
  };
  ...
  if (error) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center">
        <Link to="/">
          Something has gone wrong - click to go to homepage.
        </Link>
      </Box>
    );
  }
        ...
        <Grid item className={classes.genresContainer}>
          {data?.genres?.map((genre, id) => (
            <Link key={genre.name} className={classes.links} to="/" onClick={() => dispatch(selectGenreOrCategory(genre.id))}>
              <img src={genreIcons[genre.name.toLowerCase()]} className={classes.genreImage} height={30} alt="Icon" />
